...
class DataTransformer:
    ...
    def __init__(self, mean_values=[104.008, 116.669, 122.675], crop_size=[240, 320]):
    ...
    def scale_img(self, im, new_h, new_w, interpolation_, is_depth, scale=0):
        ...
        if not is_depth:
            # scale input image
            return cv2.resize(im, (new_w, new_h), interpolation=interpolation_);
        else:
            return cv2.resize(im, (new_w, new_h), interpolation=interpolation_) / float(scale); 
    ...
    def random_crop_img(self, scaled_img, rand_y, rand_x):        
        ...
        if len(scaled_img.shape) == 3:
            return scaled_img[rand_y:rand_y+self.crop_size_h, rand_x:rand_x+self.crop_size_w, :];
        else:
            return scaled_img[rand_y:rand_y+self.crop_size_h, rand_x:rand_x+self.crop_size_w];
            
             
    def preprocess(self, im, depth_label, scale):
        """
        ...
        depth_label = depth_label / float(10000);
        interpolation_ = cv2.INTER_LINEAR;
        ...
        #print(im.shape[0])
        #print(im.shape[1])
        #print(self.crop_size_h)
        #print(self.crop_size_w)
        rand_y = random.randint(0, im.shape[0] - self.crop_size_h);
        rand_x = random.randint(0, im.shape[1] - self.crop_size_w);
        # rgb
        ...
        depth_label = depth_label.reshape(1, self.crop_size_h, self.crop_size_w);
