...
void ContinuousMeanfieldIteration<Dtype>::OneTimeSetUp(
  ...
  if (this->blobs_.size() > 0) {
    LOG(INFO) << "Continuous Meanfield iteration skipping parameter initialization.";
  } else {
    blobs_.resize(2);
 ...
 */
template <typename Dtype>
void ContinuousMeanfieldIteration<Dtype>::PrePass(
 ...
 */
template <typename Dtype>
void ContinuousMeanfieldIteration<Dtype>::Forward_cpu() {
...
template<typename Dtype>
void ContinuousMeanfieldIteration<Dtype>::Backward_cpu() {
  ...
  for (int n = 0; n < num_; ++n) {
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, channels_, channels_,
              num_pixels_, Dtype(1.), message_passing_norm_.cpu_diff() + message_passing_norm_.offset(n),
...
INSTANTIATE_CLASS(ContinuousMeanfieldIteration);
}  // namespace caffe
