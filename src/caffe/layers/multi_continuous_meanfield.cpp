/*!
 *  \brief     A neural network implementation for continuous mean-field updating with inspiration from the CRF-RNN implementation
 *  \authors   Dan Xu
 *  \version   1.0
 *  \date      2017
...
#include "caffe/layers/multi_continuous_meanfield.hpp"
...
void MultiContinuousMeanfieldLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {

  const caffe::MultiContinuousMeanfieldParameter meanfield_param = this->layer_param_.multi_continuous_meanfield_param();
  ...
  spatial_weight_ = meanfield_param.spatial_weight();
  bilateral_weight_ = meanfield_param.bilateral_weight();
  ...
  // Initialize the parameters that will be updated by backpropagation.
  if (this->blobs_.size() > 0) {
    LOG(INFO) << "MultiContinuousMeanfield layer skipping parameter initialization.";
  } else {

    this->blobs_.resize(2);// blobs_[0]:spatial kernel weights; blobs_[1]:bilateral kernel weights
...
//    CHECK(pFile) << "The file 'spatial.par' is not found. Please create it with initial spatial kernel weights.";
    for (int i = 0; i < channels_; i++) {
        this->blobs_[0]->mutable_cpu_data()[i * channels_ + i] = spatial_weight_;
...
//    CHECK(pFile) << "The file 'bilateral.par' is not found. Please create it with initial bilateral kernel weights.";
    for (int i = 0; i < channels_; i++) {
        this->blobs_[1]->mutable_cpu_data()[i * channels_ + i] = bilateral_weight_;
...
//    fclose(pFile);
  ...
  LOG(INFO) << "Finish initialization of Spatial and Bilateral.";
  ...
  // the first unary term from bottom[0]
  split_layer_bottom_vec_.clear();
  split_layer_bottom_vec_.push_back(bottom[1]);
  ...
  //set sum of weights of two unaries for normalizing in each meanfiled interation
  sum_multiplier_.Reshape(1, channels_, height_, width_);
  caffe_set(count_ / num_, Dtype(1.), sum_multiplier_.mutable_cpu_data());

  LOG(INFO) << "Finish Initialization of multi_stage.";

  // Make instances of MeanfieldIteration and initialize them.
  meanfield_iterations_.resize(num_iterations_);
  for (int i = 0; i < num_iterations_; ++i) {
    meanfield_iterations_[i].reset(new ContinuousMeanfieldIteration<Dtype>());
    meanfield_iterations_[i]->OneTimeSetUp(
        split_layer_out_blobs_[i].get(), // the first unary term
        //split_layer_out_blobs_[i + num_iterations_ +  1].get(), // the second unary term
        (i == 0) ? bottom[0] : iteration_output_blobs_[i - 1].get(), // U_0
        ...
        &sum_multiplier_); //for matrix calculation in the meanfield normalization
  }

  this->param_propagate_down_.resize(this->blobs_.size(), true);

  LOG(INFO) << ("MultiContinuousMeanfieldLayer initialized.");
}

template <typename Dtype>
void MultiContinuousMeanfieldLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
	// Do nothing.
}
 ...
 *
 * bottom[0] - the hidden variables
 * bottom[1] - the observation variables
 * bottom[2] - RGB images
 *
 * top[0] - Output of the mean field inference.
 */
template <typename Dtype>
void MultiContinuousMeanfieldLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {

  split_layer_bottom_vec_[0] = bottom[1];
 ...
 */
template<typename Dtype>
void MultiContinuousMeanfieldLayer<Dtype>::Backward_cpu(
  ...
  // Accumulate diffs from mean field iterations w.r.t. the kernel weights stored in blobs_.
...
template<typename Dtype>
void MultiContinuousMeanfieldLayer<Dtype>::compute_bilateral_kernel(const Blob<Dtype>* const rgb_blob, const int n, float* const output_kernel) {
...
template <typename Dtype>
void MultiContinuousMeanfieldLayer<Dtype>::compute_spatial_kernel(float* const output_kernel) {
...
INSTANTIATE_CLASS(MultiContinuousMeanfieldLayer);
REGISTER_LAYER_CLASS(MultiContinuousMeanfield);
}  // namespace caffe
